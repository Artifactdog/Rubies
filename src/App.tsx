...
const ChevronIcon = ({ direction }: { direction: 'left' | 'right' }) => <Icon size={16}><path d={direction === 'left' ? 'm15 18-6-6 6-6' : 'm9 18 6-6-6-6'} /></Icon>

const RubyMark = () => <div className="ruby-mark" aria-hidden="true"><span /></div>

const StatusDot = ({ status }: { status: 'healthy' | 'underfunded' | 'overspent' }) => <span className={`status-dot ${status}`} aria-label={status} />

const MoneyInput = ({ value, onCommit, ariaLabel }: { value: number; onCommit: (value: number) => void; ariaLabel: string }) => {
  ...
  return <input className="money-input" aria-label={ariaLabel} inputMode="decimal" value={draft} onChange={(event) => setDraft(event.target.value)} onBlur={commit} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur() }} />
}

const DialogFrame = ({ title, subtitle, onClose, children, wide = false }: { title: string; subtitle: string; onClose: () => void; children: ReactNode; wide?: boolean }) => {
  useEffect(() => {
...
function App() {
  ...
  return <BudgetApp key={session.mode} session={session} onLock={() => setSession(null)} onPasswordChanged={(password) => { if (session.mode === 'protected') setSession({ ...session, password }) }} />
}

const AccessScreen = ({ vaultPresent, onProtected, onDemo, onDeleteVault }: { vaultPresent: boolean; onProtected: (state: BudgetState, password: string) => void; onDemo: () => void; onDeleteVault: () => void }) => {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError('')
    if (!vaultPresent && password.length < 8) { setError('Use at least 8 characters. A longer passphrase is better.'); return }
    if (!vaultPresent && password !== confirmPassword) { setError('The passwords do not match.'); return }
    setBusy(true)
    try {
      if (vaultPresent) {
  ...
  const resetVault = () => {
    if (window.confirm('Delete the encrypted budget stored on this device? This cannot be undone without an export.')) onDeleteVault()
  }
...
const BudgetApp = ({ session, onLock, onPasswordChanged }: { session: Session; onLock: () => void; onPasswordChanged: (password: string) => void }) => {
  ...
  const [moveOpen, setMoveOpen] = useState<{ from?: string | null; to?: string | null } | null>(null)
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key.toLowerCase() === 'm') setMoveOpen({})
    }
    window.addEventListener('keydown', onKeyDown)
    return () => window.removeEventListener('keydown', onKeyDown)
  }, [])
  ...
  const importData = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    try { actions.importState(JSON.parse(await file.text()) as BudgetState) }
    catch (error) { window.alert(error instanceof Error ? error.message : 'Could not import this Rubies file.') }
...
const PlanView = ({ state, readyToAssign, collapsedGroups, onToggleGroup, onMonthChange, onAssignmentChange, onEditCategory, onAddCategory, onEditGroup, onNewGroup, onNewTransaction, onMoveMoney, onFundTarget, onAutoAssign }: { state: BudgetState; readyToAssign: number; collapsedGroups: Set<string>; onToggleGroup: (groupId: string) => void; onMonthChange: (month: string) => void; onAssignmentChange: (month: string, categoryId: string, amount: number) => void; onEditCategory: (category: Category) => void; onAddCategory: (groupId: string) => void; onEditGroup: (groupId: string) => void; onNewGroup: () => void; onNewTransaction: () => void; onMoveMoney: (preset: { from?: string | null; to?: string | null }) => void; onFundTarget: (category: Category) => void; onAutoAssign: () => void }) => {
...
const AccountsView = ({ state, selectedAccountId, onSelectAccount, onNewTransaction, onEditTransaction, onAddAccount, onEditAccount }: { state: BudgetState; selectedAccountId?: string; onSelectAccount: (accountId?: string) => void; onNewTransaction: () => void; onEditTransaction: (transaction: Transaction) => void; onAddAccount: () => void; onEditAccount: (account: Account) => void }) => {
...
const TransactionDialog = ({ state, transaction, selectedAccountId, onClose, onSubmit, onDelete }: { state: BudgetState; transaction?: Transaction; selectedAccountId?: string; onClose: () => void; onSubmit: (transaction: Omit<Transaction, 'id'>) => void; onDelete?: () => void }) => {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); const parsed = Math.abs(parseMoney(amount)); if (!accountId || !payee.trim() || parsed === 0) return; onSubmit({ accountId, date, payee: payee.trim(), memo: memo.trim(), categoryId: kind === 'income' ? null : categoryId, amount: kind === 'income' ? parsed : -parsed, cleared }) }
...
const CategoryDialog = ({ state, initial, groupId, onClose, onSubmit, onArchive }: { state: BudgetState; initial?: Category; groupId: string; onClose: () => void; onSubmit: (category: Omit<Category, 'id'>) => void; onArchive?: () => void }) => {
  const [name, setName] = useState(initial?.name ?? '')
  const [selectedGroupId, setSelectedGroupId] = useState(initial?.groupId ?? groupId)
  const [targetType, setTargetType] = useState<CategoryTarget['type'] | 'none'>(initial?.target?.type ?? 'none')
  const [targetAmount, setTargetAmount] = useState(initial?.target ? (initial.target.amount / 100).toFixed(2) : '')
  const [targetMonth, setTargetMonth] = useState(initial?.target?.targetMonth ?? shiftMonth(state.activeMonth, 6))
  const [note, setNote] = useState(initial?.note ?? '')
  const submit = (event: FormEvent) => { event.preventDefault(); if (!name.trim()) return; const amount = Math.abs(parseMoney(targetAmount)); const target: CategoryTarget | undefined = targetType === 'none' || amount === 0 ? undefined : { type: targetType, amount, ...(targetType === 'by-date' ? { targetMonth } : {}) }; onSubmit({ groupId: selectedGroupId, name: name.trim(), note: note.trim(), ...(target ? { target } : {}) }) }
  return <DialogFrame title={initial ? 'Edit category' : 'Add category'} subtitle="Categories are envelopes. Targets describe how you want to fill them." onClose={onClose}><form onSubmit={submit}><div className="form-grid single-column"><label>Category name<input value={name} onChange={(event) => setName(event.target.value)} autoFocus required /></label><label>Group<select value={selectedGroupId} onChange={(event) => setSelectedGroupId(event.target.value)}>{state.groups.filter((group) => !group.hidden).map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select></label><label>Target type<select value={targetType} onChange={(event) => setTargetType(event.target.value as CategoryTarget['type'] | 'none')}><option value="none">No target</option><option value="monthly-spending">Refill balance to an amount each month</option><option value="monthly-savings">Assign a fixed amount each month</option><option value="by-date">Save a total amount by a date</option></select></label>{targetType !== 'none' && <label>{targetType === 'by-date' ? 'Total target amount' : 'Monthly amount'}<input inputMode="decimal" value={targetAmount} onChange={(event) => setTargetAmount(event.target.value)} placeholder="0.00" required /></label>}{targetType === 'by-date' && <label>Target month<input type="month" min={state.activeMonth} value={targetMonth} onChange={(event) => setTargetMonth(event.target.value)} required /></label>}<label>Notes <span className="optional">Optional</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Purpose, rules, or a reminder" rows={3} /></label></div><footer className="dialog-actions split-actions">{onArchive ? <button type="button" className="danger-button" onClick={() => { if (window.confirm('Archive this category? Existing history will be kept.')) onArchive() }}>Archive</button> : <span />}<div><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-button">{initial ? 'Save category' : 'Add category'}</button></div></footer></form></DialogFrame>
}

const MoveMoneyDialog = ({ state, readyToAssign, preset, onClose, onMove }: { state: BudgetState; readyToAssign: number; preset: { from?: string | null; to?: string | null }; onClose: () => void; onMove: (from: string | null, to: string | null, amount: number) => void }) => {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (valid) onMove(sourceId, destinationId, parsed) }
...
const GroupDialog = ({ initialName, onClose, onSubmit }: { initialName: string; onClose: () => void; onSubmit: (name: string) => void }) => {
  ...
  return <DialogFrame title={initialName ? 'Rename group' : 'New category group'} subtitle="Use groups to organize related envelopes." onClose={onClose}><form onSubmit={(event) => { event.preventDefault(); if (name.trim()) onSubmit(name.trim()) }}><div className="form-grid single-column"><label>Group name<input value={name} onChange={(event) => setName(event.target.value)} autoFocus required /></label></div><footer className="dialog-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit">Save group</button></footer></form></DialogFrame>
}

const AccountDialog = ({ initial, onClose, onSubmit }: { initial?: Account; onClose: () => void; onSubmit: (account: Omit<Account, 'id'>, openingBalance: number) => void }) => {
  const [name, setName] = useState(initial?.name ?? '')
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (name.trim()) onSubmit({ name: name.trim(), type, onBudget, closed }, parseMoney(openingBalance)) }
