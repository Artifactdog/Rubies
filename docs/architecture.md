...
Rubies 2026.08.06.1 is a static React PWA served by Nginx. It requires no external cloud account, database, or runtime service.

### Layers

- `src/domain.ts` — version-3 data model, integer-money calculations, target recommendations, Ready to Assign, migration, and nYNAB import
...
- `src/App.tsx` — password gate, responsive workspace, mouse allocation controls, dialogs, import, and demo mode
...
Ready to Assign is uncategorized account cash flow through the selected month minus cumulative assignments through that month.
...
Version-2 vault payloads are normalized to version 3 when unlocked.
